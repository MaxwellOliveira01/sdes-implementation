...
vector<int> ep = {4, 1, 2, 3, 2, 3, 4, 1}; // expansion/permutation

vector<int> sbox_permutation = {2, 4, 3, 1}; // S-box permutation
...
// string apply_fk(string mask, string subkey) {
    // assert((int)mask.size() == 8);
    // return apply_fk(mask.substr(0, 4), mask.substr(4), subkey);
// }
...
string encrypt(string plaintext, string key) {
    assert((int)plaintext.size() == BLOCK_SIZE);
    assert((int)key.size() == 10);

    auto subkey1 = get_first_subkey(key);
    auto subkey2 = get_second_subkey(key);

    auto permuted_text = apply_identity_permutation(plaintext);
...
string decrypt(string ciphertext, string key) {
    assert((int)ciphertext.size() == BLOCK_SIZE);
    assert((int)key.size() == 10);

    auto subkey1 = get_first_subkey(key);
    auto subkey2 = get_second_subkey(key);

    auto permuted_text = apply_identity_permutation(ciphertext);
...
// posso criar uma classe EBC e CBC que herdam de algo de uma classe comum ´Cipher´ our ´Des´ ou ´SDES´
// e tem dois metodos abstratos: encrypt e decrypt que recebem um bloco de texto
// o construtor dessas classes já recebe o tamanho do bloco e a chave
// posso fazer um padding simpleszao, apenas completa com zeros

// o sdes em si cifra apenas um bloco
// o EBC e CBC vão cifrar um texto inteiro
